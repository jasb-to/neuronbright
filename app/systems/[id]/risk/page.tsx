...
export default function SystemRiskPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [assessment, setAssessment] = useState<RiskAssessment>(fallbackAssessment);
  const [missingControls, setMissingControls] = useState(0);
  ...
  useEffect(() => {
    ...
    params.then(async ({ id }) => {
      ...
      const response = await fetch(`/api/systems/${id}/governance`, { cache: "no-store" });
      ...
      if (data.risk) {
        setAssessment({
          ...
          dimensions: data.risk.dimensions ?? [],
          assessedAt: data.risk.assessed_at,
        });
      }
      setMissingControls((data.controls ?? []).filter((control: { status: string }) => control.status !== "Complete").length);
    });
    return () => { active = false; };
  }, [params]);
