...
export default function SystemPage({
  ...
  useEffect(() => {
    ...
    async function loadSystem() {
      ...
      const saved = getStoredSystem(id);
      setSystem(
        saved ?? aiSystems.find((item) => item.id === id) ?? aiSystems[0],
      );
    }
  ...
  if (!system) {
    return (
      <AppShell>
        <div className="p-8 text-sm text-white/40">Loading system...</div>
      </AppShell>
    );
  }
    ...
    <AppShell>
      <div className="mx-auto max-w-[1300px] px-6 py-8 xl:px-8">
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[1fr_420px]">
          <div className="space-y-6">
            ...
            <section className="rounded-xl border border-white/[0.08] bg-[#0b0b0b]">
              <div className="flex items-center justify-between border-b border-white/[0.08] p-5">
                <div>
                  <h2 className="text-sm font-medium">Governance workflow</h2>
                  <p className="mt-1 text-xs text-white/25">
                    Controls generated from the initial assessment
                  </p>
                </div>
                <ShieldAlert size={18} className="text-[#dc6b27]" />
              </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <CheckCircle2
                        size={16}
                        className={
                          i < 2 ? "text-[#dc6b27]" : "text-white/20"
                        }
                      />
                      <span className="text-sm text-white/65">{x}</span>
                    </div>
          ...
          <div className="space-y-6">
            <section className="rounded-xl border border-[#dc6b27]/25 bg-[#dc6b27]/[0.05] p-6">
              <p className="text-[9px] uppercase tracking-[0.15em] text-[#dc6b27]">
                Evidence posture
              </p>
              <p className="mt-3 text-4xl font-semibold">{system.evidence}%</p>
              <p className="mt-2 text-xs text-white/30">Evidence verified</p>
              <div className="mt-6 h-1.5 overflow-hidden rounded-full bg-white/[0.06]">
                <div
                  className="h-full rounded-full bg-[#dc6b27]"
                  style={{ width: `${system.evidence}%` }}
                />
              </div>
            </section>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-white/30">
                    Review the risk assessment and map the required controls.
                  </p>
                </div>
              </div>
              <Link
                href={`/systems/${system.id}/risk`}
                className="mt-5 inline-flex w-full items-center justify-center rounded-lg bg-[#dc6b27] px-4 py-2.5 text-xs font-semibold text-black hover:opacity-90"
              >
