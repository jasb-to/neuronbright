      ...
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-10 md:grid-cols-[1.4fr_2fr]">
          <div>
            ...
            <p className="mt-5 text-[10px] uppercase tracking-[0.18em] text-white/20">
              Birmingham, UK
            </p>
          </div>
          ...
          <div className="flex flex-wrap content-start gap-x-7 gap-y-4 md:justify-end">
            {links.map(([label, href]) => (
              <Link key={href} href={href} className="text-xs text-white/35 transition-colors hover:text-white">
                {label}
              </Link>
            ))}
          </div>
        ...
        <div className="mt-10 flex flex-col gap-3 border-t border-white/[0.06] pt-5 text-[10px] text-white/20 sm:flex-row sm:items-center sm:justify-between">
          <span>© {new Date().getFullYear()} NEURONBRIGHT. All rights reserved.</span>
