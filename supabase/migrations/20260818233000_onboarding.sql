create or replace function public.bootstrap_organisation(
  ...
  organisation_industry text default 'General business',
  organisation_size text default 'Unknown'
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  values (
    ...
    coalesce(nullif(trim(organisation_industry), ''), 'General business'),
    coalesce(nullif(trim(organisation_size), ''), 'Unknown'),
